...
class SocketService with ChangeNotifier {
  ...
  void disconnect() {
    if (_socket != null) {
      ...
      _eventControllers.forEach((_, controller) => controller.close());
  ...
  void on(String event, Function(dynamic) callback) {
    if (!_eventControllers.containsKey(event)) {
      _eventControllers[event] = StreamController.broadcast();
      _socket?.on(event, (data) => _eventControllers[event]!.add(data));
    }
    _eventControllers[event]!.stream.listen(callback);
  }
  ...
  void _registerSocketEvents() {
    _socket!.onConnect((_) {
      log.info('Socket connected successfully: ${_socket!.id}');
      notifyListeners();
    });
      ...
      if (context != null) {
        showDialog(
          ...
          builder: (context) => AlertDialog(
            title: Text('Incoming Call from ${data['from']}'),
    ...
    _eventControllers.forEach((event, controller) {
